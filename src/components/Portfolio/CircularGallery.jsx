...
function createTextTexture(
  gl,
  text,
  font = '600 22px "Neue Montreal", ui-sans-serif, system-ui',
  color = "#111"
) {
...
class Title {
  ...
  createMesh() {
    ...
    const textH = this.plane.scale.y * 0.13;
    const textW = textH * aspect;

    this.mesh.scale.set(textW, textH, 1);
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textH * 0.55 - 0.06;
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          p.z = (sin(p.x * 4.0 + uTime) * 1.2 + cos(p.y * 2.0 + uTime) * 1.2) * (0.08 + uSpeed * 0.45);
...
class App {
  constructor(
    container,
    {
      ...
      // ✅ NEW: ultra slow auto drift
      autoMove = true,
      autoSpeed = 14, // pixels/second-ish (keep this SMALL: 8–22)
      autoPauseMs = 900, // pause after user input
      wheelEnabled = true,
      wheelMultiplier = 0.20, // keep small
    } = {}
  ) {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    const galleryItems = items?.length ? items : fallback;
  ...
  _pauseAuto() {
    this.autoPauseUntil = performance.now() + this.autoPauseMs;
  }
  ...
  onWheel(e) {
    ...
    this.scroll.target +=
      (delta > 0 ? this.scrollSpeed : -this.scrollSpeed) * this.wheelMultiplier;
  ...
  update() {
    ...
    // ✅ SUPER slow auto drift (only when not interacting)
    if (this.autoMove && !this.isDown && now > this.autoPauseUntil) {
      // move a tiny amount each frame
      // (autoSpeed is "px/sec-ish", dt converts it to per-frame)
...
export default function CircularGallery({
  ...
  // ✅ NEW props (premium auto drift)
  autoMove = true,
  autoSpeed = 6,     // try 8–18 for "really slow"
  autoPauseMs = 900,  // pause after user input
  wheelEnabled = false, // since you wanted scroll disabled
  wheelMultiplier = 0.18,
}) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ([entry]) => app.setRunning(entry.isIntersecting),
      { threshold: 0.12 }
    );
